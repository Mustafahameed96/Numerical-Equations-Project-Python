# -*- coding: utf-8 -*-
"""
Created on Tue Apr 17 18:55:39 2018
...
#Declaring initial variables
    ...
    xlist = np.linspace(-0.4, 0.3, 100)
    ylist = np.linspace(-0.4, 0.3, 100)
...
#First Derivative i.e function'x
            ...
            var1 = ( ((x+h)**3) + (2*(x+h)*y) + (2*(y)**2) - (2*(x+h)**2) + (y) + 8 )
            var2 = ( ((x-h)**3) + (2*(x-h)*y) + (2*(y)**2) - (2*(x-h)**2) + (y) + 8 )
            
            fx = (var1 - var2)/(2 * h)
            
#First Derivative i.e function'y
            var1 = ( ((x)**3) + (2*x*(y+k)) + (2*(y+k)**2)  - (2*(x)**2) + (y+k) + 8)
            var2 = ( ((x)**3) + (2*x*(y-k)) + (2*(y-k)**2)  - (2*(x)**2) + (y-k) + 8)
            
            fy = (var1 - var2)/(2 * k)
            
#Second Derivative i.e function"x
            var1 = ( ((x+h)**3) + (2*(x+h)*y) + (2*(y)**2) - (2*(x+h)**2) + (y) + 8)
            var2 = ( ((x)**3)   + (2*(x)*y)   + (2*(y)**2) - (2*(x)**2)   + (y) + 8)
            var3 = ( ((x-h)**3) + (2*(x-h)*y) + (2*(y)**2) - (2*(x-h)**2) + (y) + 8)
...
#Second Derivative i.e function"y
            var1 = ( ((x)**3) + (2*x*(y+k)) + (2*(y+k)**2) - (2*(x)**2) + (y+k) + 8)
            var2 = ( ((x)**3) + (2*(x)*y)   + (2*(y)**2)   - (2*(x)**2) + (y)   + 8)
            var3 = ( ((x)**3) + (2*x*(y-k)) + (2*(y-k)**2) - (2*(x)**2) + (y-k) + 8)
            
            fyy = (var1 - (2 * var2) + var3)/((k)**2)
            
#Second Derivative i.e function"xy
            var1 = ( ((x+h)**3)  + (2*(x+h)*(y+k)) + (2*(y+k)**2) - (2*(x+h)**2) + (y+k) + 8)
            var2 = ( ((x+h)**3)  + (2*(x+h)*(y-k)) + (2*(y-k)**2) - (2*(x+h)**2) + (y-k) + 8)            
            var3 = ( ((x-h)**3)  + (2*(x-h)*(y+k)) + (2*(y+k)**2) - (2*(x-h)**2) + (y+k) + 8)         
            var4 = ( ((x-h)**3)  + (2*(x-h)*(y-k)) + (2*(y-k)**2) - (2*(x-h)**2) + (y-k) + 8)
